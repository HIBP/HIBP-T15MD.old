...
if __name__ == '__main__':
    ...
    dEbeam = 20.
    Ebeam_range = np.arange(80., 260. + dEbeam, dEbeam)  # [keV]
#    Ebeam_range = np.arange(60., 400. + dEbeam, dEbeam)  # [keV]
#    Ebeam_range = np.arange(240., 1600. + dEbeam, dEbeam)  # [keV]
...
# %%
    ...
    plot_scan_xy(traj_list_passed, r_aim, A2_edges, B2_edges, 260., Btor, Ipl)
    plot_scan_xz(traj_list_passed, r_aim, A2_edges, B2_edges, 260., Btor, Ipl)


    fig, ax1, figure_name = plot_grid_xy(traj_list_passed, r_aim, Btor, Ipl, marker_E='')
    figlegend = plot_legend(ax1, figure_name)

    save_png([fig, figlegend], [figure_name, 'Legend_for_' + figure_name])
